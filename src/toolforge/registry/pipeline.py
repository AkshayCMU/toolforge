...
def build_registry(
    ...
    # -------------------------------------------------------------------------
    # Stage 3: Semantic typing (LLM, cached)
    # -------------------------------------------------------------------------
    log.info("build.stage", stage="semantic_typing")
    typed_tools, accepted_new = type_corpus(subset, client, cache_dir)

    # Count typing cache hits/misses by comparing pre/post (type_corpus logs internally)
    # We re-scan to get exact counts for the build report
    ...
    for tool in subset:
        for ep in tool.endpoints:
            ...
            if typing_load_cache(cache_path) is not None:
                ...
                typing_calls += 1  # shouldn't happen on warm build

    # -------------------------------------------------------------------------
    # Stage 4: Schema inference (static + schema + LLM, cached)
    # -------------------------------------------------------------------------
    log.info("build.stage", stage="schema_inference")
    final_tools, schema_stats = infer_corpus(typed_tools, examples_dir, client, cache_dir)
    ...
    # Chain-only types = types that appear as response fields but NEVER as params.
    # These can only be obtained from a prior tool call — the executor uses this list
    # to enforce hard grounding for CHAIN_ONLY-class arguments.
    chain_only_types = sorted(
        st for st in field_type_counts
        if st not in param_type_counts and st in accepted_vocab
    )
